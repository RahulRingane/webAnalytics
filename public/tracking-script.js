...
(function () {
  ...
  function trackPageView(customData) {
    ...
    // Avoid duplicate pageview if same URL within 1 second
    if (currentUrl === lastTrackedUrl && now - lastTrackedTime < 1000) return;

    window.your_tracking.pageview(customData);
  ...
  function initialize() {
    ...
    // Track first pageview after setup
    setTimeout(() => {
      trackPageView();
    }, 200);
