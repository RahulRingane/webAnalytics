    ...
    for (const check of toFlush) {
      try {
        const project = await prisma.project.findFirst({
          where: { domain: check.url.replace(/^https?:\/\//, "") },
        });
        if (!project) continue;
        ...
        if (!isUp) {
          await prisma.incident.create({
            data: {
              projectId: project.id,
              startTime: new Date(),
              reason: "Uptime check failed",
            },
          });
        } else {
          const unresolved = await prisma.incident.findFirst({
            where: { projectId: project.id, resolved: false },
          });
          if (unresolved) {
            const endTime = new Date();
            const duration =
              (endTime.getTime() - unresolved.startTime.getTime()) / 1000;
