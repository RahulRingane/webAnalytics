...
export  async function GET(req: NextRequest,
    ...
    try {
        ...
        if (typeof id !== "string") {
            return NextResponse.json(
                { message: "Invalid Id Parameter" }, { status: 400 }
            )
        }
        ...
        if (!project) {
            return NextResponse.json(
                { message: "monitor not found" }, { status: 404 }
            )
        }
...
export async function PUT(req: NextRequest,
    ...
    if (!session) {
        return NextResponse.json(
            { user: null, message: "Unauthorized", success: false }, { status: 403 }
        );
    }

    const { name, url, method, interval, timeout, expectedStatusCode, active } = body;
...
export async function DELETE(req: NextRequest,
    ...
    try {
        ...
        return NextResponse.json(
            { message: "Project deleted successfully", deletedProject: result }, { status: 200 }
        )
    } catch (error) {
        return NextResponse.json(
